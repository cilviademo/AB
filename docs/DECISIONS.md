...
## D-028 — Reference library (B6): entry types follow the context tags; isolation is byte-identity; actions are explicit
An entry's type is derived from its `usage_context` (USER_RECOVERY / UNKNOWN_CONTEXT → USER_ARTIFACT, KNOWN_SOURCE_FIXTURE and SOURCE_AVAILABLE_REFERENCE → KNOWN_SOURCE_FIXTURE, BLACK_BOX_REFERENCE → BLACK_BOX_REFERENCE); FRAMEWORK_REFERENCE rows are the knowledge base's KNOWN_FRAMEWORK / KNOWN_THIRD_PARTY / KNOWN_SHARED_INTERNAL fingerprint sets and its shared implementations; DSP_REFERENCE rows are the 33 clean-room components of `reference/music_reference_corpus_21` (MIT, per-file sha256, evidence CANDIDATE — never proof). A known-source fixture is *resolved* only when a `fixture.json` manifest names the job's binary hash; a job merely tagged as a fixture says so ("declared, not proven"). "External fixture source stays isolated from user recoveries" is checked as byte-identity: no file under a user or black-box project's `04_reconstruction/` may equal a fixture `Source/` file (> 64 bytes). The four actions are RPC calls the user makes; `reference.set_context` re-types a job explicitly, keeps `context_history` in the manifest and changes no stage output (C1: interpretation only). The library is written to `<workspace>/Reference/` (REFERENCE_LIBRARY.md + reference_library.json, contract `artifactbench.reference_library`). Reversible: the knowledge tables were not changed; the `artifact.ownership` column keeps carrying `usage_context` (D-026).

## D-029 — Hardening pass (B7–B9): round-trip semantics, the unseen-fixture rule, and what a re-typing may touch
*Export round-trip* restores state, never recomputes it: `bundle.import` recreates the project from `evidence/`, `validation/`, the reconstruction and handoff files and restores the stage records from `evidence/00_manifest/stages.json` (cache keys included, so an unchanged input is reused on the other machine). What the export did not carry stays absent and is reported (`missing_evidence_dirs`); an import never invents a stage result. *Unseen fixture* (B8 test 1): the first blind run is recorded as-is in STATUS — including its failing row — before anything is changed; a fix that follows is marked as such and the fixture is thereafter no longer unseen (a new one is needed for the next blind test). *Re-typing* (`reference.set_context`) changes the job row and the manifest's context fields plus a `context_history`, never a stage output; a manifest older than the current contract is left untouched and the change goes to `00_manifest/context_history.json`. *Job upsert* updates in place (`ON CONFLICT DO UPDATE`): `INSERT OR REPLACE` cascaded a job's inputs away through the foreign keys, which the B9 black-box run exposed. *Fresh-machine statuses* `AVAILABLE | MISSING | WRONG_VERSION | UNSUPPORTED` sit beside the doctor's PASS/WARNING/UNAVAILABLE/FAIL verdicts (both kept: the verdict says whether AB can run, the status says what to install). Reversible: all are additive; the manifest and stage contracts are unchanged.

## D-030 — Consistency pass: the frozen v2 licensing wording is translated at install, never left for an agent to read
The frozen Static Recovery v2 port (diff-baseline) still labels licensing classes `PROTECTED_SUBSYSTEM` and writes an agent prompt saying "document its architecture, do not reimplement or bypass it" — the pre-Addendum-C rule. Rather than edit the frozen engine, the STATIC install step (`static.api.canonicalize_v2`, STAGE_VERSION 3) translates its output before anything current reads it: generated text (agent_prompt.md, HANDOFF.md, scaffold headers, README_RECOVERY.md) gets the Addendum C sentence and the canonical label; `RecoveredScaffolds/Protected/` installs as `RecoveredScaffolds/Licensing/`; the v2 reconstruction index entries get `role` = canonical with `original_role` = `PROTECTED_SUBSYSTEM` kept for provenance; every translated class is listed in `01_evidence/rtti/role_canonicalization.json`. Frozen evidence JSON (`recovery.*` contracts, e.g. `01_evidence/rtti/classes.json`) keeps the original label: it is evidence, and the alias table (`roles.canonical_role`) reads it. AB's own writers (HANDOFF/TODO) and the Bench view print canonical roles with the v2 label as a tooltip. Classification of every occurrence in the repository: frozen-v2 alias (generate.ts / bundle.ts / static-engine test — allowed, translated at install), legacy test input (test_reconstruct / test_decompile / test_transform_licensing — allowed, they prove the alias), stale user-facing requirement (AB_BRIEF "no bypass; clean replacement", SPEC title/input wording "owned", README "owned", pyproject "owned plugin recovery" — changed), stale generated instruction (the v2 agent prompt — translated at install; the Python writer already used the new rule at export), stale documentation (BLOCKERS iPlug2 "PROTECTED_SUBSYSTEM stub", iPlug2 fixture comment and README `--ownership OWNED`, the Corpus view's "Ownership" column — changed). Regression: engine/tests/test_licensing_handoff.py asserts that no agent prompt, HANDOFF, TODO, reconstruction index or exported bundle contains a prohibition and that the legacy label is accepted as input but canonical in every current output. Reversible: the translation is a pure function of the installed text; the original engine output is reproducible from the frozen port.
