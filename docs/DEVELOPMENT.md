...
- Recovery goal and naming are RECONSTRUCT options: `ab-cli run <job> --stage RECONSTRUCTION_COMPLETE --option goal=MODERNIZE --option naming=CANONICALIZE_NAMES --option preserve_dsp_behavior=YES` (ADDENDUM C3, docs/NAMING_CANONICALIZATION.md). `PRESERVE_ORIGINAL` (default) builds `Source/Active` from `recovered_source/`; `MODERNIZE` / `REFACTOR` from `transformed_source/`; `MIGRATE` / `PORT` / `REBUILD` are recorded NOT_AVAILABLE. COMPARE then fills `04_reconstruction/transformation_graph.json` and stores the built variant's renders under `06_validation/<variant>_behavior/`. Context tags at ingest: `ab-cli ingest --context USER_RECOVERY --source-availability SOURCE_UNKNOWN …` (C1; wording only).
...
- Reference library (ADDENDUM B6): `ab-cli reference --text` lists every typed entry with provenance and the fixture-source isolation check; `--provenance <name|fingerprint id>` answers "where did AB learn this?"; `--dashboard <job>` prints the ground-truth counts; `--set-context <job> <CONTEXT>` re-types a job explicitly (Add as Fixture / Use as Black-Box Reference). Written to `<workspace>/Reference/`.
- Second ground-truth fixture (ADDENDUM B8 test 1, *unseen*): `fixtures/groundtruth_unseen/build_all.sh` (JUCE shared with the first fixture) → `out/stripped/ABUnseen.vst3`; score a blind run with `ab-cli ground-truth <job> --truth fixtures/groundtruth_unseen/truth.json --phase 4`. The evaluator reads every expectation (classes, fit family, decoy field, licensing) from the fixture's `truth.json`, never from code. Acceptance tests live in `engine/tests/test_acceptance_*.py` (mixed drop, corpus); the unseen-fixture and source+binary runs are recorded in docs/STATUS.md (B8).

- Export round-trip (ADDENDUM B7): `ab-cli export <job> --zip` writes `<Plugin>_RECOVERED/` with `evidence/00_manifest/stages.json`; `ab-cli import <folder|zip> [--replace]` recreates the project and job (stage records, cache keys, byte-identical evidence) on any workspace without re-running a stage.
